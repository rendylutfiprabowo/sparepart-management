...
class sampleContoller extends Controller
{
    public function store($id_project,$id_history)
    {
        ...
        foreach ($scopes as $scope) {
            if(isset($validated['check'][$scope->nama_scope])){
                $sample = new sample();
                $sample->id_sample = $faker->numberBetween(100, 999);;
                $sample->status_sample = false;
                $sample->id_history = $id_history;
                $sample->id_scope = $scope->id_scope;
                $sample->save();
                $form_sample = NULL;
                if(!$scope->detailed){
                    $form_sample = new formReport();
                    $form_sample->id_formreport = $sample->id_sample.'-'.$scope->id_scope;
                    $form_sample->field_formreport = $scope->form->field_form;
                    $form_sample->id_sample = $sample->id_sample;
                    $form_sample->id_lab = '1';
                    $form_sample->save();
                }
                else{
                    $form_sample = new formReport();
                    ...
                    $data = json_decode($scope->form->field_form,true);
                    foreach ($data as $key => $field) {
                        if(!isset($validated['check'][$scope->nama_scope][$key])){
                            $data[$key] = '-1' ;
                        }
                    }
                    $form_sample->field_formreport = json_encode($data);
                    $form_sample->id_sample = $sample->id_sample;
                    $form_sample->id_lab = '1';
                    $form_sample->save();
                }
